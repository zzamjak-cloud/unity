// CAT SoftMask 지원을 추가한 UIEffect 셰이더 오버라이드
...
Shader "Hidden/UI/Default (UIEffect)"
{
    Properties
    {
        ...
        // CAT SoftMask 프로퍼티 (UIEffectSoftMaskProxy.cs가 프로그래매틱으로 설정)
        [HideInInspector] _MaskTex("Soft Mask Texture", 2D) = "white" {}
        [HideInInspector] _Softness("Softness", Float) = 0.1
        ...
        [HideInInspector] _MaskTex2("Soft Mask Texture 2", 2D) = "white" {}
        [HideInInspector] _Softness2("Softness 2", Float) = 0.1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // 항상 include: cginc 내부에서 _CAT_SOFTMASK 미정의 시 빈 매크로로 폴백
            #include "CAT_SoftMask_Core.cginc"
            // ==== CAT SOFTMASK END ====
