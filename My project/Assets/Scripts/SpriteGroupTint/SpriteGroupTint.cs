...
#if UNITY_EDITOR
        // 안전하게 SetDirty 호출 (DontSaveInEditor 플래그가 있는 오브젝트는 건너뜀)
        private void SafeSetDirty(UnityEngine.Object obj)
        {
            if (obj == null) return;
            
            // DontSaveInEditor 플래그가 설정된 오브젝트는 SetDirty 호출 시 에러 발생
            // HideFlags를 확인하여 저장 가능한 오브젝트만 처리
            if ((obj.hideFlags & HideFlags.DontSaveInEditor) == 0)
            {
                ...
                {
                    // SetDirty 실패 시 무시 (런타임 생성 오브젝트 등)
                }
            }
        }
#endif
