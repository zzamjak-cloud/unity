    ...
    public class TMPCharacterAnimation : MonoBehaviour
    {
        ...
        private void OnValidate()
        {
            ...
            if (_enableLoop && _loopCount == -1 && _enableDisappear)
            {
                ...
                Debug.Log("[TMPCharacterAnimation] Loop Count가 무한(-1)이므로 Disappear Animation이 자동으로 비활성화되었습니다.");
            }

            // Loop Count가 0이면 Loop 비활성화 안내
            if (_enableLoop && _loopCount == 0)
            {
                Debug.Log("[TMPCharacterAnimation] Loop Count가 0이므로 Loop Animation이 실행되지 않습니다. 1 이상으로 설정하세요.");
            }
        }
#endif
        ...
        private void ApplyShadowMesh(TMP_Text tmpText, TMPOutlineEffect outlineEffect)
        {
            ...
            if (s_shadowVertexCache.Count > 0)
            {
                ...
                if (_shadowMesh == null)
                {
                    ...
                    _shadowMesh.name = "TMPCharacterAnimation_Shadow";
