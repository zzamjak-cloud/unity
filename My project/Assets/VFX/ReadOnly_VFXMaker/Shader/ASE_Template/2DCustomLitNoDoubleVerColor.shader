Shader /*ase_name*/ "Hidden/Universal/2D Custom Lit No Double VerColor" /*end*/
{
	Properties
	{
		/*ase_props*/
		...
		*/
		...
		Pass
		{
			...
			half4 frag( VertexOutput IN /*ase_frag_input*/ ) : SV_TARGET
			{
				...
				//***여기서부터 Premultiplied를 위한 커스텀 처리***
				//BlendOp Add, Max 방식이 더 깔끔해서 임시로 남겨둠
				//이 설정을 하려면 Blend RGB를 Premultiplied 로 하고 Dst에 _BlendMode 연결은 그대로

				//color.rgb *= color.a; // 색상 합성을 위해 알파를 RGB에 미리 곱함 (Premultiplied)

				// 데스크톱 투과 방지 핵심: 
				// Additive(더하기) 모드일 때는 알파 값을 0으로 만들어 윈도우 투명도에 영향을 주지 않게 하고,
				// AlphaBlend 모드일 때만 최소한의 가시성을 위해 알파를 유지하거나 1로 고정할 수 있습니다.
				//if (_BlendMode == 1.0) // Additive 모드일 경우
				//{
				//	color.a = 0; // RGB는 살아있으므로 검은 배경에 더해지지만, 윈도우를 불투명하게 만들지 않음
				//}
				
				// color *= unity_SpriteColor; //색상이 여러번 곱해지기에 지움
				return color;
			}
